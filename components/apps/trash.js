...
export class Trash extends Component {
  constructor() {
    ...
    this.trashItems = [
      {
        name: "Experiment crypto integration",
        icon: "./themes/filetypes/js.png",
      },

      {
        name: "Singapore vacation.zip",
        icon: "./themes/filetypes/zip.png",
      },
      {
        name: "AlpacaLLM Fine tune 2",
  ...
  render() {
    const emptyTrashClassName = `border border-black bg-black bg-opacity-50 px-3 py-1 my-1 mx-1 rounded ${
      this.state.empty ? "rounded text-gray-300" : "hover:bg-opacity-80"
    }`;
    const restoreTrashClassName = `border border-black bg-black bg-opacity-50 px-3 py-1 my-1 mx-1 rounded ${
      this.state.empty ? "hover:bg-opacity-80" : "rounded text-gray-300"
    }`;
    ...
    return (
      <div className="w-full h-full flex flex-col bg-ub-cool-grey text-white select-none">
        <div className="flex items-center justify-between w-full bg-ub-warm-grey bg-opacity-40 text-sm">
          ...
          <div className="flex">
            <div onClick={this.restoreTrash} className={restoreTrashClassName}>
              Restore
            </div>
            <div onClick={this.emptyTrash} className={emptyTrashClassName}>
              Empty
            </div>
          </div>
        </div>
